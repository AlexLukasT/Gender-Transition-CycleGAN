...
cyclegan = framework.CycleGAN(batch_size=BATCH_SIZE,
                              gen_steps=GEN_STEPS,
                              critic_steps=CRITIC_STEPS,
                              gp_weight=GP_WEIGHT,
                              cycle_weight=CYCLE_WEIGHT)

cyclegan.build(gen_lr=GEN_LR,
               critic_lr=CRITIC_LR)

data_x = data.get_dataset(shuffle=True, male=True)
data_y = data.get_dataset(shuffle=True, male=False)
